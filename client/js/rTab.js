...
* 定义类
*/
const RTab = function RTab(options, pageData, pageItemData) {
...
RTab.prototype = {
  ...
  setPageData(pageId, data) {
    this.pageData[pageId] = data;
  },
  /**
    * 删除页面数据
    */
  removePageData(pageId) {
    delete this.pageData[pageId];
  },
  /**
    * 获取item数据
    */
  getItemData(itemId) {
    return this.pageItemData[itemId];
  },
  /**
    * 设置item数据
    */
  setItemData(itemId, data) {
    this.pageItemData[itemId] = data;
  },
  /**
    * 删除item数据
    */
  removeItemData(itemId) {
    delete this.pageItemData[itemId];
  },
  /**
    * 切换到页面面板
    */
  switchToPage(pageId) {
    // 获得数据
    const data = this.pageData[pageId] || {};
  ...
  switchToText(itemId) {
    ...
    // 获取数据
    let data = this.pageItemData[itemId];
    if (!data) { // 默认值
      data = {
        fontSize: '12px',
        color: '000000',
  ...
  sortPageItems() {
    ...
    Object.keys(this.pageItemData).forEach((key) => {
      const item = self.pageItemData[key];
      const { pageId } = item;
      if (!result[pageId]) {
        result[pageId] = [];
      }
  ...
  buildData() {
    ...
    $('.page-wrap').each(function onEach() {
      ...
      const pageObj = self.pageData[key];
